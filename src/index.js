...
import * as convexHull from './hull/hull.js';
...
export class MERmaid extends React.Component {
    ...
    loadData() {
		...
		const cb = function(data_string, mythis) {
			logTime('parsing data');

			const data = d3.csvParseRows(data_string);
			// data[1]: eg. ['-957.2759', '-930.179', 'ADAM9', '12']

			logTime('setting data');
			// header: eg ['x', 'y', 'gene0', 'cell']
			const header = data.splice(0, 1)[0];
			console.log('header', header);

			const validHeaders = ['x', 'y', 'z', 'cell', 'process'];
			const options = header.filter(h => !validHeaders.includes(h));
			// options: ['gene0']

			let opts = {};
			d3.map(options, function(option) {
				let opsSet = new Set();
				const j = header.indexOf(option);
				for (let i = 0; i < data.length; i++) {
					const optionVal = data[i][j]; //eg. 'gene0'
					opsSet.add(optionVal);
				}
				// ops will have gene names
				let ops = Array.from(opsSet).sort();
			...
			let zArr = [];
			data.forEach((row) => {
				xArr.push(parseInt(row[xIdx]));
				yArr.push(parseInt(row[yIdx]));
				zArr.push(parseInt(row[zIdx]));
			});
			let originX = d3.mean(xArr)/DATA_SCALE;
			let originY = d3.mean(yArr)/DATA_SCALE;
			let originZ = d3.mean(zArr)/DATA_SCALE;
			...
			// set random color for each gene0 dropdown
			const selected = {}
			...
			const dataSubset = data.filter(d => d[header.indexOf('cell')] > 0);
			...
			let hull = d3.nest()
				.key(function (d) {
					return d[header.indexOf('cell')].concat('-', d[header.indexOf('process')]);
				})
				.rollup(function (v) {
		...
		var cb2 = function(data_string, mythis) {
			logTime('parsing data');

			var data = d3.csvParseRows(data_string);
...
class Menu extends React.Component {
	...
	return (
		...
		<select id={'Option' + this.props.id}
				style={styles.label}
